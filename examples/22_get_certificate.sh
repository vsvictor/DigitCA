...
ISSUE_RESPONSE=$(curl "${CURL_OPTS[@]}" -X POST "${API_BASE}/api/v1/certificates" \
  -H "$AUTH_HEADER" -H "Content-Type: application/json" \
  -d '{"common_name":"get-sample.internal","profile":"server-tls","issuer":"root","dns_names":["get-sample.internal"],"ip_sans":[],"validity_days":30}')
SERIAL=$(echo "$ISSUE_RESPONSE" | sed -n 's/.*"serial":"\([^"]*\)".*/\1/p')

[[ -n "$SERIAL" ]] || { echo "Cannot parse serial"; exit 1; }

curl "${CURL_OPTS[@]}" "${API_BASE}/api/v1/certificates/${SERIAL}" -H "$AUTH_HEADER"
echo
