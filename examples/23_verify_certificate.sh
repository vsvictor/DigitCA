...
ISSUE_RESPONSE=$(curl "${CURL_OPTS[@]}" -X POST "${API_BASE}/api/v1/certificates" \
  -H "$AUTH_HEADER" -H "Content-Type: application/json" \
  -d '{"common_name":"verify-sample.internal","profile":"server-tls","issuer":"root","dns_names":["verify-sample.internal"],"ip_sans":[],"validity_days":30}')
SERIAL=$(echo "$ISSUE_RESPONSE" | sed -n 's/.*"serial":"\([^"]*\)".*/\1/p')
[[ -n "$SERIAL" ]] || { echo "Cannot parse serial"; exit 1; }

curl "${CURL_OPTS[@]}" "${API_BASE}/api/v1/certificates/${SERIAL}/verify" -H "$AUTH_HEADER"
echo
